...
const ContextGraph = lazy(() =>
  import("@/components/context-graph").then((module) => ({
    default: module.ContextGraph,
  })),
);
const TrendStrip = lazy(() =>
  import("@/components/trend-strip").then((module) => ({
    default: module.TrendStrip,
  })),
);
...
function PanelPlaceholder({
  ...
  return (
    <div
      className={`flex items-center justify-center rounded-3xl border border-white/10 bg-slate-950/80 p-6 text-center shadow-[0_16px_60px_rgba(15,23,42,0.5)] ${className}`}
    >
...
export default function App() {
  ...
  const setSelectedCommunityId = useCommandCenterStore(
    (state) => state.setSelectedCommunityId,
  );
  const setSelectedQueryId = useCommandCenterStore(
    (state) => state.setSelectedQueryId,
  );
  const setSearchTerm = useCommandCenterStore((state) => state.setSearchTerm);
  const resetSelections = useCommandCenterStore(
    (state) => state.resetSelections,
  );

  const selectedQuery = useMemo(
    () =>
      demoDataset.queries.find((query) => query.id === selectedQueryId) ??
      demoDataset.queries[0],
    [selectedQueryId],
  );
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-4 py-6 lg:px-6">
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-4xl">
              ...
              <h1 className="text-3xl font-semibold tracking-tight text-white md:text-5xl">
                {demoDataset.headline}
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 md:text-base">
                {demoDataset.subheadline}
              </p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3 xl:min-w-[540px]">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-lg font-semibold text-white">
                  {new Date(demoDataset.generatedAt).toLocaleString()}
                </p>
                <p className="mt-2 text-xs text-slate-400">
                  Refresh window for all synthetic graph events and demo
                  provenance labels.
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-lg font-semibold text-white">
                  {selectedCommunity?.title ?? "All communities"}
                </p>
                <p className="mt-2 text-xs text-slate-400">
                  {selectedNode
                    ? `Node: ${selectedNode.label}`
                    : "Select a node or cluster to tighten the graph narrative."}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {demoDataset.metrics.map((metric) => {
            ...
            return (
              <article
                key={metric.id}
                className={`rounded-3xl border p-4 shadow-[0_12px_50px_rgba(15,23,42,0.45)] ${toneClasses(metric.tone)}`}
              >
            ...
            <section className="rounded-3xl border border-white/10 bg-slate-950/80 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.5)]">
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-3">
                ...
                <p className="mt-3 text-xs text-slate-400">
                  Preset queries drive the highlighted graph path; search
                  broadens visible node labels.
                </p>
              </div>
          ...
          <Suspense
            fallback={
              <PanelPlaceholder
                ...
                detail="Preparing Sigma.js scene and graph reducers."
              />
            }
          >
            <ContextGraph
            ...
            <section className="rounded-3xl border border-white/10 bg-slate-950/80 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.5)]">
              ...
              {selectedNode ? (
                <div className="mt-4 space-y-3">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full border px-2.5 py-1 text-[11px] ${statusClasses(selectedNode.status)}`}
                      >
                        {selectedNode.status}
                      </span>
                    </div>
                    <p className="mt-3 text-xs text-slate-400">
                      {provenanceLine(selectedNode)}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-4 rounded-2xl border border-dashed border-white/15 bg-white/5 p-5 text-sm text-slate-400">
                  Click a graph node to inspect its evidence, provenance, and
                  neighboring path structure.
                </div>
              )}
            </section>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <Suspense
            fallback={
              <PanelPlaceholder
                title="Loading trend telemetry"
                detail="Preparing lightweight live-style metric strips."
                className="h-[360px]"
              />
            }
          >
          ...
          <section className="rounded-3xl border border-white/10 bg-slate-950/80 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.5)]">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="text-lg font-semibold text-white">
                  Construction + query event stream
                </h2>
              </div>
              ...
              {demoDataset.activities.map((activity) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] ${statusClasses(activity.status)}`}
                    >
                      {activity.status}
                    </span>
                  </div>
              ...
              {demoDataset.timeline.map((event) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] ${severityClasses(event.severity)}`}
                    >
                      {event.severity}
                    </span>
                  </div>
              ...
              {demoDataset.sources.map((source) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] ${statusClasses(source.status)}`}
                    >
                      {source.status}
                    </span>
                  </div>
                  ...
                  <p className="mt-3 text-xs text-slate-500">
                    {source.provenance.sourceSystem} · {source.freshness} ·
                    confidence {Math.round(source.provenance.confidence * 100)}%
                  </p>
                </article>
              ))}
            </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-950/80 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.5)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h2 className="text-lg font-semibold text-white">
                Searchable graph entities
              </h2>
              <p className="mt-2 text-sm text-slate-400">
                Use search or direct selection to jump around the knowledge
                graph without leaving the dashboard.
              </p>
            </div>
            ...
            {filteredNodes.map((node) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`rounded-full border px-2.5 py-1 text-[11px] ${statusClasses(node.status)}`}
                  >
                    {node.status}
                  </span>
                </div>
